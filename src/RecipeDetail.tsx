...
import { meals, ingredients } from "./mockData";
import "./Recipes.css";
...
export function RecipeDetail() {
  ...
  return (
    <main className="recipe-detail">
      <h2>{recipe.name}</h2>
      {recipe.imageUrl ? (
        <img src={recipe.imageUrl} alt={recipe.name} className="recipe-img-detail" />
      ) : (
        <div className="recipe-img-empty">Chybí obrázek</div>
      )}


      <p><strong>Typ jídla:</strong> {recipe.types.join(", ")}</p>
      <p><strong>Porce:</strong> {recipe.servings}</p>
